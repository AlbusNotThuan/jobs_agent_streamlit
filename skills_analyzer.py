...
csv_file_path = r"c:\Users\Dang\Desktop\jobs_crawler\tools_LLM\2025-07-30_020028_linkedin_jobs.csv"
def extract_in_demand_skills(top_n: int = 20, min_frequency: int = 2) -> dict:
    """Extracts the most in-demand skills from LinkedIn jobs CSV data.

    Args:
        top_n: Number of top skills to retrieve (default: 20)
        min_frequency: Minimum frequency of skills to be counted (default: 2)

    Returns:
        A dictionary containing skills statistics.
    """
    print(f"[DEBUG] Called extract_in_demand_skills with csv_file_path={csv_file_path}, top_n={top_n}, min_frequency={min_frequency}")
    try:
        # Read CSV file
...
def categorize_skills(skills: list) -> dict:
    """Categorizes a list of skills into main categories.

    Args:
        skills: List of skill names to categorize.

    Returns:
        A dictionary mapping category names to lists of skills.
    """
    print(f"[DEBUG] Called categorize_skills with skills={skills}")
    categories = {
...
def analyze_skills_trends(date_column: str = 'Posted_Date') -> dict:
    """Analyzes skills trends over time from a CSV file.

    Args:
        csv_file_path: Path to the CSV file.
        date_column: Name of the date column (default: 'Posted_Date').

    Returns:
        A dictionary with skills trends statistics by month/year.
    """
    print(f"[DEBUG] Called analyze_skills_trends with csv_file_path={csv_file_path}, date_column={date_column}")
    try:
        df = pd.read_csv(csv_file_path, encoding='utf-8')
...
def get_hot_skills_last_month(date_column: str = 'Posted_Date', top_n: int = 15) -> dict:
    """Finds the most in-demand skills in the last month from the CSV file.

    Args:
        csv_file_path: Path to the CSV file.
        date_column: Name of the date column (default: 'Posted_Date').
        top_n: Number of hot skills to retrieve (default: 15).

    Returns:
        A dictionary containing hot skills with detailed statistics.
    """
    print(f"[DEBUG] Called get_hot_skills_last_month with csv_file_path={csv_file_path}, date_column={date_column}, top_n={top_n}")
    try:
        df = pd.read_csv(csv_file_path, encoding='utf-8')
...
def get_skills_by_category(category_type: str = 'all', top_n: int = 10) -> dict:
    """Gets skills filtered by a specific category from the CSV file.

    Args:
        csv_file_path: Path to the CSV file.
        category_type: Category type (e.g., 'programming', 'ai_ml', 'data', 'cloud', 'soft_skills', 'all').
        top_n: Number of skills to retrieve for each category (default: 10).

    Returns:
        A dictionary containing skills by the requested category.
    """
    print(f"[DEBUG] Called get_skills_by_category with csv_file_path={csv_file_path}, category_type={category_type}, top_n={top_n}")
    try:
        df = pd.read_csv(csv_file_path, encoding='utf-8')
...
def get_trending_skills_comparison(days_back: int = 30) -> dict:
    """Compares skills trends from the last N days from the current time.

    Args:
        csv_file_path: Path to the CSV file.
        days_back: Number of days to analyze from current time (default: 30).

    Returns:
        A dictionary containing skills trends analysis.
    """
    print(f"[DEBUG] Called get_trending_skills_comparison with csv_file_path={csv_file_path}, days_back={days_back}")
    try:
        df = pd.read_csv(csv_file_path, encoding='utf-8')
...
def get_job_categories_analysis() -> dict:
    """Analyzes skills by job categories from the CSV file.

    Args:
        csv_file_path: Path to the CSV file.

    Returns:
        A dictionary with job categories and their top skills.
    """
    print(f"[DEBUG] Called get_job_categories_analysis with csv_file_path={csv_file_path}")
    try:
        df = pd.read_csv(csv_file_path, encoding='utf-8')
...
def print_skills_report(skills_data: dict) -> None:
    """Prints a formatted skills report for the analysis results.
    ...
    Returns:
        None
    """
    print(f"[DEBUG] Called print_skills_report with skills_data keys={list(skills_data.keys()) if skills_data else skills_data}")
    if not skills_data:
...
def print_hot_skills_report(hot_skills_data: dict) -> None:
    """Prints a hot skills report for the last month.

    Args:
        hot_skills_data: Dictionary containing hot skills statistics.

    Returns:
        None
    """
    print(f"[DEBUG] Called print_hot_skills_report with hot_skills_data keys={list(hot_skills_data.keys()) if hot_skills_data else hot_skills_data}")
...
def print_skills_by_category_report(category_data: dict, category_type: str = 'all') -> None:
    """Prints a report of skills by category.

    Args:
        category_data: Dictionary containing categorized skills.
        category_type: The category type to display (default: 'all').

    Returns:
        None
    """
    print(f"[DEBUG] Called print_skills_by_category_report with category_type={category_type}, category_data keys={list(category_data.keys()) if category_data else category_data}")
...
def print_trending_report(trending_data: dict) -> None:
    """Prints a skills trends report for the analyzed period.

    Args:
        trending_data: Dictionary containing trending skills analysis.

    Returns:
        None
    """
    print(f"[DEBUG] Called print_trending_report with trending_data keys={list(trending_data.keys()) if trending_data else trending_data}")
...
def demo_new_features():
    """Demonstrates new features of the skills analyzer.
    ...
    Returns:
        None
    """
    print(f"[DEBUG] Called demo_new_features")
...
def export_to_csv(skills_data, output_file):
    ...
    Returns:
        None
    """
    print(f"[DEBUG] Called export_to_csv with output_file={output_file}, skills_data keys={list(skills_data.keys()) if skills_data else skills_data}")
    try:
...
def quick_analysis():
    print(f"[DEBUG] Called quick_analysis")
